...
var require_polyfills = __commonJS((exports2, module2) => {
  ...
  function patch(fs5) {
    ...
    if (!fs5.lchmod) {
      fs5.lchmod = function(path11, mode, cb) {
        if (cb)
          process.nextTick(cb);
      };
    ...
    function patchLchmod(fs6) {
      fs6.lchmod = function(path11, mode, callback) {
        fs6.open(path11, constants.O_WRONLY | constants.O_SYMLINK, mode, function(err, fd) {
          if (err) {
            if (callback)
              callback(err);
            return;
      ...
      fs6.lchmodSync = function(path11, mode) {
        var fd = fs6.openSync(path11, constants.O_WRONLY | constants.O_SYMLINK, mode);
        var threw = true;
        var ret;
        try {
    ...
    function patchLutimes(fs6) {
      if (constants.hasOwnProperty("O_SYMLINK")) {
        fs6.lutimes = function(path11, at, mt, cb) {
          fs6.open(path11, constants.O_SYMLINK, function(er, fd) {
            if (er) {
              if (cb)
                cb(er);
              return;
        ...
        fs6.lutimesSync = function(path11, at, mt) {
          var fd = fs6.openSync(path11, constants.O_SYMLINK);
          var ret;
          var threw = true;
          try {
...
var require_graceful_fs = __commonJS((exports2, module2) => {
  ...
  function patch(fs6) {
    ...
    function createReadStream(path11, options) {
      return new fs6.ReadStream(path11, options);
    }
    function createWriteStream(path11, options) {
      return new fs6.WriteStream(path11, options);
    }
  ...
  update(pythonSettings) {
    ...
    this.linting = this.linting ? this.linting : {
      ...
      pylintCategorySeverity: {
        convention: import_coc2.DiagnosticSeverity.Hint,
        error: import_coc2.DiagnosticSeverity.Error,
        fatal: import_coc2.DiagnosticSeverity.Error,
        refactor: import_coc2.DiagnosticSeverity.Hint,
        warning: import_coc2.DiagnosticSeverity.Warning
      },
      pep8CategorySeverity: {
        E: import_coc2.DiagnosticSeverity.Error,
        W: import_coc2.DiagnosticSeverity.Warning
      },
      flake8CategorySeverity: {
        E: import_coc2.DiagnosticSeverity.Error,
        W: import_coc2.DiagnosticSeverity.Warning,
        F: import_coc2.DiagnosticSeverity.Warning
      },
      mypyCategorySeverity: {
        error: import_coc2.DiagnosticSeverity.Error,
        note: import_coc2.DiagnosticSeverity.Hint
      },
    ...
    const isort = systemVariables.resolveAny(pythonSettings.get("isort"));
    if (this.sortImports) {
...
var Pylint = class extends BaseLinter {
  ...
  async hasConfigurationFile(folder) {
    ...
    if (await import_fs_extra2.default.pathExists(path7.join(folder, pylintrc)) || await import_fs_extra2.default.pathExists(path7.join(folder, dotPylintrc))) {
      return true;
    }
    let current = folder;
    let above = path7.dirname(folder);
    do {
      if (!await import_fs_extra2.default.pathExists(path7.join(current, "__init__.py"))) {
        break;
      }
      if (await import_fs_extra2.default.pathExists(path7.join(current, pylintrc)) || await import_fs_extra2.default.pathExists(path7.join(current, dotPylintrc))) {
        return true;
      }
      current = above;
    ...
    if (await import_fs_extra2.default.pathExists(path7.join(home, dotPylintrc))) {
      return true;
    }
    if (await import_fs_extra2.default.pathExists(path7.join(home, ".config", pylintrc))) {
      return true;
    }
    if (!this.isWindows) {
      if (await import_fs_extra2.default.pathExists(path7.join("/etc", pylintrc))) {
        return true;
      }
    }
    return false;
...
var Pytype = class extends BaseLinter {
  ...
  async hasConfigurationFile(folder) {
    if (await import_fs_extra3.default.pathExists(path8.join(folder, pytypecfg))) {
      return true;
    }
    let current = folder;
    let above = path8.dirname(folder);
    do {
      if (!await import_fs_extra3.default.pathExists(path8.join(current, "__init__.py"))) {
        break;
      }
      if (await import_fs_extra3.default.pathExists(path8.join(current, pytypecfg))) {
        return true;
      }
      current = above;
